...
#如果只是修改或删除了已被git管理的文档, 没必要使用git add
...
```shell
#-a 不经过 stage,直接提交所有更改到 repo
# commit 意味着一次版本更新

git commit -am "commit commnets"   

#每一次提交对应一个 git 系统中唯一的 commit ID(相当于项目版本号), commit ID用以标识每一次更新
...
```shell
# git show 查看每一次 commit 的具体细节
git show 0d189
git show HEAD      #当前分支的最新版本更新细节
```
